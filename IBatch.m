...
for i=1:13 %dataset No
    for j=1:3 % prior and truncate
        for k=1:5 % divergences
            for l=1:3 % dimension
                [err,ttl,mis_rate,p_err]=IProcess(file(i).filename,file(i).ext,file(i).sorting_column,file(i).sorting_method,file(i).num_of_class,file(i).row_bound,file(i).column_bound,balance_set(j).prior,balance_set(j).truncate,file(i).dim,dim(l).val,method(k).name,optimizer.step,optimizer.tolerance,optimizer.method)
                Err(1,q)=err(1);Err(2,q)=err(2);
                Ttl(1,q)=ttl(1);Ttl(2,q)=ttl(2);
                Mis(1,q)=mis_rate(1);Mis(2,q)=mis_rate(2);Mis(3,q)=mis_rate(3);
                PErr(q)=p_err;
                Txt(q)=file(i).text+" "+balance_set(j).text+" "+method(k).text+" "+dim(l).text;
                q=q+1;
            end
        end
    end
...
xlswrite("Test_Result_I.xlsx",result);