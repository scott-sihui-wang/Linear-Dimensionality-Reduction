...
for test=1:100
    ...
    train_set=sortrows(train_set,8,'ascend');
    x1=zeros(1,7);
    x2=zeros(1,7);
    x3=zeros(1,7);
    A=zeros(7);
    ...
    while train_set(i,8)==1
        x1=x1+train_set(i,1:7);
        cnt1=cnt1+1;
        i=i+1;
    end
    x1=x1./cnt1;
    while train_set(i,8)==2
        x2=x2+train_set(i,1:7);
        cnt2=cnt2+1;
        i=i+1;
    end
    x2=x2./cnt2;
    while i<=length(train_set) && train_set(i,8)==3
        x3=x3+train_set(i,1:7);
        cnt3=cnt3+1;
        i=i+1;
    end
    x3=x3./cnt3;
    for i=1:length(train_set)
        if train_set(i,8)==1
            t=train_set(i,1:7)-x1;
            A=A+t'*t;
        elseif train_set(i,8)==2
            t=train_set(i,1:7)-x2;
            B=B+t'*t;
        elseif train_set(i,8)==3
            t=train_set(i,1:7)-x3;
            C=C+t'*t;
        end
    end
    ...
    for i=1:length(test_set)
        ...
        elseif test_set(i,8)==2
            if(l2<l1 || l2<l3)
                err2=err2+1;
            end
        elseif test_set(i,8)==3
            if(l3<l1 || l3<l2)
                err3=err3+1;
            end
        end
