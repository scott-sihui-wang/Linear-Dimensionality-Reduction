...
problem.costgrad = @(M) IG_D3(alpha,lambda,x1,x2,M);
...
[x3, xcost, info, options] = steepestdescent(problem,y,options);
...
r_sigma3=sqrt(proj3'*cov*proj3);
r_mean3=proj3'*mu;

d31=sqrt((r_mean3)^2/2+(1+r_sigma3)^2);
d32=sqrt((r_mean3)^2/2+(1-r_sigma3)^2);
dist3=(d31+d32)/(d31-d32);
...
function [f,g]=IG_D3(alpha,lambda,x1,x2,M)
    f=-(1+0.5*alpha'*M*M'*alpha)/sqrt(M'*lambda*M)-sqrt(M'*lambda*M);
    egrad=-(M'*alpha)/sqrt(M'*lambda*M)*alpha+(1+1/2*alpha'*M*M'*alpha)/(sqrt(M'*lambda*M))^3*lambda*M-lambda*M/sqrt(M'*lambda*M);
    g1=egrad-(egrad'*M)/(M'*M)*M;
    g2=g1-(g1'*x1)/(x1'*x1)*x1;
    g=g2-(g2'*x2)/(x2'*x2)*x2;
end