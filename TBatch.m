...
for i=1:13
    [means,covs,prior,data,row_bound,column_bound] = preprocessing(file(i).filename,file(i).ext,file(i).sorting_column,file(i).sorting_method,file(i).num_of_class,file(i).row_bound,file(i).column_bound,file(i).dim,true,false);
    proj=FID(means,covs,prior,"TV",file(i).dim,r_dim,optimizer.step,optimizer.tolerance,optimizer.method);
    for j=1:r_dim
        x=proj(:,1:j);
        ...
        PErr(q)=p_err;
        dim_info=sprintf("Dimension=%d",j);
        Txt(q)=file(i).text+" "+dim_info;
        q=q+1;
    end
end
...
xlswrite("Test_Result_TV2.xlsx",result);